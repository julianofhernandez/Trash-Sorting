...
def ssd_preds(images, process_online, single_classification):
    ...
    if process_online and len(images) == 1:
        ...
        res = requests.post('http://localhost:5000/read/batch-inference/apple', data={'num_image': len(byte_arrs)}, files=byte_arrs)
        if res.status_code == 200:
            res = json.loads(res.content)
            if res['error_code'] == 0:
                res = res['batch_predictions']
                if single_classification:
                    res = [[preds[0]] for preds in res]#May need to adjust this
            else:
                return None
        else:
            return None
