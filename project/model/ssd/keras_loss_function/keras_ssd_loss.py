...
#         localization_loss = tf.to_float(self.smooth_L1_loss(y_true[:,:,-12:-8], y_pred[:,:,-12:-8])) # Output shape: (batch_size, n_boxes)
        classification_loss = tf.cast(self.log_loss(y_true[:,:,:-12], y_pred[:,:,:-12]), tf_float_type) # Output shape: (batch_size, n_boxes)
        localization_loss = tf.cast(self.smooth_L1_loss(y_true[:,:,-12:-8], y_pred[:,:,-12:-8]), tf_float_type) # Output shape: (batch_size, n_boxes)

        # 2: Compute the classification losses for the positive and negative targets.

        # Create masks for the positive and negative ground truth classes.
        negatives = y_true[:,:,0] # Tensor of shape (batch_size, n_boxes)
        positives = tf.cast(tf.reduce_max(y_true[:,:,1:-12], axis=-1), tf_float_type) # Tensor of shape (batch_size, n_boxes)
        ...
        def f2():
            ...
            negatives_keep = tf.cast(tf.reshape(negatives_keep, [batch_size, n_boxes]), tf_float_type) # Tensor of shape (batch_size, n_boxes)
        ...
        total_loss = total_loss * tf.cast(batch_size, tf_float_type)
