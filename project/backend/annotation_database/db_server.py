...
"""
Creates a blueprint object so different Flask apps may know how to serve different routes
"""
...
def create_db():
    """
    Creates a SQLite database table called "image_data" with the following columns:
        - id            the id or name of the image. Automatically generated by db 
...
def get_max_entries():
    """
    Retrieves the maximum value of the "id" column in the "image_data" table of the SQLite database.
    Returns 0 if the "id" column is empty. Used for generating image names/ids
    """
    global IMAGE_DATA_DB
    conn = sqlite3.connect(IMAGE_DATA_DB)
    cursor = conn.cursor()
...
def setup():
    """
    The setup for the annotation database
    """
...
def invalid_request(error_msg='Invalid Key', error_code=1, code=401):
    """
    This function returns a JSON response indicating that the request is invalid. 
...
def handle_entry():
    ...
    This function handles a POST request to create a new entry in the database.
    It checks for a valid developer key, an image file, and various form fields (
    annotation, num_annotations, dataset, and metadata). 
    If all fields are present, it saves the image file to the images directory, 
    inserts the data into the image_data table, and returns a JSON response with the new entry ID, 
    error message (if any), and error code.
    """
...
def handle_get_entry_image(id):
    """
    This function handles a GET request to retrieve an image file with the specified ID. 
    It reads the image file from the images directory and returns it as a response.
    """
