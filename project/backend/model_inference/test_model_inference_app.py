...
class TestCreate:
    def test_invalid_key(self, client):
        res = client.post('http://localhost:5000/create/model/test',
                            data={'key': 'wrongkey',
    ...
    def test_already_exists(self, client):
        res = client.post('http://localhost:5000/create/model/test',
                            data={'key': 'secretkey',
                                  ...
                                  'model': open('README.md', 'rb')})
        assert res.status_code == 200
        assert res.get_json() == {'error_code': 5,
                                           'error_msg': 'Model already exists',
                                           'successful': False}

    def test_missing_model(self, client):
        res = client.post('http://localhost:5000/create/model/test2',
                            data={'key': 'secretkey',
                                  'metadata': json.dumps({'a': 5})})
        assert res.status_code == 200
    ...
    def test_missing_metadata(self, client):
        ...
        assert res.status_code == 200
        assert res.get_json() == {'error_code': 4,
    ...
    def test_new_success(self, client):
        res = client.post('http://localhost:5000/create/model/test2',
                            data={'key': 'secretkey',
                                  'metadata': json.dumps({'b': 6}), 'model': open('README.md', 'rb')})
        assert res.status_code == 200
...
class TestReadBatchInf:
    def test_missing_images(self, client):
        ...
        res, res.get_json()
        assert res.status_code == 200
        assert res.get_json() == {'batch_predictions': None,
                                           'error_code': 3,
                                           'error_msg': 'Missing image_0 in files part of request',
                                           'model_name': 'test'}

    def test_missing_num_image(self, client, tmp_img):
        res = client.post('http://localhost:5000/read/batch-inference/test',
...
class TestUpdate:
    def test_invalid_key(self, client):
        res = client.put('http://localhost:5000/update/model/test',
    ...
    def test_model(self, client):
        res = client.put('http://localhost:5000/update/model/test2',
                           data={'key': 'secretkey', 'model': open('README.md', 'rb')})
        assert res.status_code == 200
        assert res.get_json() == {'error_code': 0,
                                           'error_msg': None,
    ...
    def test_metadata(self, client):
        ...
        assert res.get_json() == {'error_code': 0,
                                           'error_msg': None,
                                           'successful': True}
        res = requests.put('http://localhost:5000/update/model/test',
