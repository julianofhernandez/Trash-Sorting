...
# from model_inference.model import preds
try:
...
MODELS_DIR = "models/"  # os.path.join(dirname, 'models/')
MODELS = {}
DEV_KEY = None
METADATAS_DIR = "metadatas/"  # os.path.join(dirname, 'metadatas/')
...
def handle_model_create(model_name):
    ...
    if "key" in request.form and request.form["key"] == DEV_KEY:
        ...
        if model_name in MODELS:
            ...
            try:
                ...
                model.save(path)
                with open(
                    os.path.join(METADATAS_DIR, model_name + ".json"), "w"
                ) as file:
    ...
    return (
        jsonify({"successful": False, "error_msg": "Invalid Key", "error_code": 1}),
        200,
    )
...
def handle_model_list():
    ...
    try:
        for metadata_file in os.listdir(METADATAS_DIR):
            ...
            model_list.append(
                {"model_name": metadata_file.rsplit(".", 1)[0], "metadata": metadata}
            )
    except Exception as e:
        error_msg = str(e)
        error_code = 2
    ...
    return (
        jsonify(
            {"model_list": model_list, "error_msg": error_msg, "error_code": error_code}
        ),
        200,
    )
...
def handle_download_model(model_name):
    ...
    return jsonify({"error_msg": "Model Not Found", "error_code": 3}), 200
...
def handle_metadata(model_name):
    ...
    return (
        jsonify(
            {"metadata": "", "error_msg": "Model Metadata Not Found", "error_code": 3}
        ),
        200,
    )
...
def handle_update_model(model_name):
    ...
    if "key" in request.form and request.form["key"] == DEV_KEY:
        ...
        return jsonify(result), 200
    result = {"successful": False, "error_msg": "Invalid Key", "error_code": 1}
    return jsonify(result), 200
